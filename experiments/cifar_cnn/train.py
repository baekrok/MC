...
parser.add_argument('--optimizer', type=str, default='sgd', choices=['sgd', 'adam', 'adagrad', 'rmsprop'])
...
run_name = f"arch-{args.arch}_opt-{args.optimizer}_data-{args.dataset}_lr-{args.learning_rate}_bsz-{args.batch_size}_wd-{args.decay}"
wandb.init(project=f'matrix_completion_rank_resnet_{args.dataset}', name=run_name, config=args)
...
def main():
    ...
    if args.optimizer == "sgd":
        ...
        optimizer = torch.optim.Adam(net.parameters(), state['learning_rate'], betas=(state['momentum'], 0.99),
                                    weight_decay=state['decay'])
    elif args.optimizer == 'adagrad':
        optimizer = torch.optim.Adagrad(net.parameters(), state['learning_rate'], weight_decay=state['decay'])

    elif args.optimizer == 'rmsprop':
        optimizer = torch.optim.RMSprop(net.parameters(), state['learning_rate'], momentum=state['momentum'], weight_decay=state['decay'])
